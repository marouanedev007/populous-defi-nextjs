...
//pares Number
const parseNumber = (amount, decimal) => {
...
export const makeRepay = async (data, base, callback) => {
  ...
  try {
    ...
    const reserves = await lendingPoolContract.methods
...
export const makeWithdraw = async (data, base, callback) => {
  ...
  try {
    ...
    const pTokenContract = new web3.eth.Contract(abi.pTokenABI, pTokenAddress);

    const amount_ = parseNumber(amount, decimals);
    const reserves = await pTokenContract.methods
      .redeem(amount_)
      .send({ from: account.address, value: 0 });
...
export const makeRewardsClaim = async (data, base, callback) => {
  ...
  try {
    const reserves = await rewardPoolContract.methods
      .getReward()
      .send({ from: account.address, gas: 2500000 });
    callback(null, reserves);
  } catch (ex) {
    return callback(ex);
  }
...
export const makeFaucet = async (data, base, callback) => {
  ...
  try {
    ...
    const reserves = await testFaucetContract.methods
      .mint(reserve)
      .send({ from: account.address, value: 0 });
...
export const createProposal = async (data, base, callback) => {
  ...
  try {
    const reserves = await governanceV2Contract.methods
      .create(
        executor,
        targets,
        values,
...
export const submitVote = async (data, base, callback) => {
  ...
  try {
    const result = await governanceV2Contract.methods
...
export const getReserveData = async (data, base, callback) => {
  ...
  try {
    ...
    reserveData.isReserveEnabled = await lendingPoolConfiguratorContract.methods
      ...
      .call({ account: account.address });
    reserveData.balanceDecreaseAllowed =
      await lendingPoolDataProviderContract.methods
        .balanceDecreaseAllowed(reserve, account.address, userReserveData[0])
        .call({ account: account.address });
    ...
    reserveData.rewardPoolAddress =
      await rewardPoolAddressManagerContract.methods
        .getRewardPool(reserve)
        .call({ account: account.address });

    reserveData.reserveAddress = reserve;
    reserveData.address = account;
    reserveData.liquidityRate = (reserveData.liquidityRate / 10 ** 27).toFixed(
      2
    );
